...
function loadProducts(pageNumber = 1) {
  ...
  const queryParams = new URLSearchParams({
    ...
    category: filterSettings.category.join(","),
  ...
  fetch(`../assets/php/search_product.php?${queryParams.toString()}`)
    ...
    .then((apiData) => {
      ...
      const productDetailPageURL =
        userMode === "user" ? "../pages/SanPham1.php" : "../pages/SanPham.html";

      let productsHTML = "";

      productList.forEach((product) => {
        const formattedPrice = Number(product.selling_price).toLocaleString(
          "vi-VN",
          {
...
function getSelectedFilters() {
  const selectedPrices = [...document.querySelectorAll(".price:checked")].map(
  ...
  return {
    ...
    category: selectedCategories, // Lọc theo danh mục
    sort: selectedSort,
  };
}
...
// Thêm event listener cho category checkbox
