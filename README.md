...
- **Trang chủ & Cửa hàng:** Hiển thị danh sách sản phẩm trực quan, phân trang và load dữ liệu động (`html4.html`).
- **Tìm kiếm & Lọc nâng cao:** Tìm kiếm sản phẩm theo tên, khoảng giá, thương hiệu và danh mục .
- **Chi tiết sản phẩm:** Xem thông tin chi tiết, kiểm tra số lượng tồn kho theo thời gian thực trước khi thêm vào giỏ.
- **Giỏ hàng & Thanh toán:** Quản lý giỏ hàng, quy trình checkout mượt mà và xác nhận đặt hàng thành công.
- **Quản lý tài khoản:** Đăng ký, đăng nhập, xem thông tin tài khoản và theo dõi lịch sử đơn hàng.
...
- **Quản lý Kho hàng:** Cốt lõi của hệ thống, quản lý phiếu nhập hàng , tính toán giá vốn bình quân và quản lý tồn kho chặt chẽ .
- **Quản lý Đơn hàng:** Xem chi tiết đơn hàng, cập nhật trạng thái đơn hàng và phân loại .
- **Quản lý Người dùng:** Danh sách khách hàng và quản trị viên, thêm mới tài khoản từ trang admin.

## 💻 Công nghệ sử dụng

- **Ngôn ngữ Backend:** PHP (Vanilla)
- **Cơ sở dữ liệu:** MySQL (`database_structure.sql`)
...
│
├── database_structure.sql  # File cấu trúc (structure only)
├── database_data.sql       # File dữ liệu (data only)
...
Để chạy dự án này trên môi trường local, bạn cần cài đặt **XAMPP**

**Bước 1: Clone dự án**
Clone hoặc copy thư mục dự án vào thư mục `htdocs` của XAMPP:

```bash
\xampp\htdocs\duanvipnhatthegioi
```
...
3. Tạo một cơ sở dữ liệu mới (ví dụ: `quebshop2`).
4. Chuyển sang tab **Import**, chọn file `database_structure.sql` nằm ở thư mục gốc của dự án và nhấn **Go** để thực thi.

**Bước 3: Cấu hình kết nối CSDL**
Tìm đến file kết nối cơ sở dữ liệu trong source code (nằm ở `assets/php/db.php`) và cập nhật các thông số cho phù hợp với localhost:
...
- **Tồn kho & Giá vốn:** Không được phép chỉnh sửa trực tiếp thông qua chức năng sửa sản phẩm . Mọi thay đổi về số lượng và giá trị tồn kho bắt buộc phải thông qua luồng **Nhập hàng** để đảm bảo hệ thống kế toán chính xác.
